...
# @author  : Mo
# @function: train model of layoutlmv3
...
from layoutlmv3.model.modeling_layoutlmv3 import LayoutLMv3ForSequenceClassification
from layoutlmv3.model.configuration_extra import LAY_CUDA_VISIBLE_DEVICES
from layoutlmv3.model.preprocess_ocr import apply_ocr
...
path_pickle_train = "train.pickle"  # 中间结果
...
use_toy = False  # True  # 只使用batch*2个数据集
...
def processor(img_or_path, img_size=224, max_len=512, padding="max_length", truncation=True):
    ...
    # text_output = tokenizer(text="".join(texts), padding=padding, truncation=truncation, max_length=max_len, stride=0)
    ...
    encoded_inputs["pixel_values"] = img.tolist()
    # encoded_inputs = {"pixel_values": img.tolist(), "bbox": bboxs}
    # encoded_inputs.update(text_output)
...
def preprocess_data(examples):
    ...
    for k, v in encoded_inputs_total.items():
        ...
        # print(len(v))
        if k == "pixel_values":
            encoded_inputs_total[k] = torch.tensor(v, dtype=torch.float32).contiguous()  # .to(device)
        else:
    ...
    tensor_data = TensorDataset(encoded_inputs_total["pixel_values"], encoded_inputs_total["input_ids"], encoded_inputs_total["attention_mask"],
...
features = Features({
    'pixel_values': Array3D(dtype="int64", shape=(ch, input_size, input_size)),
...
model = LayoutLMv3ForSequenceClassification.from_pretrained(model_name_or_path, num_labels=len(id2label))
model.to(device)
...
# shell
# nohup python tet_train.py > doc.train.layoutlmv3.log 2>&1 &
# tail -n 1000  -f doc.train.layoutlmv3.log
# |myz|

# lr = 1e-5  # 
# lr = 2e-5  # 
# lr = 3e-5  # 100%, 1epoch, 1error
# lr = 5e-5  # 100%, 1epoch, 1error
