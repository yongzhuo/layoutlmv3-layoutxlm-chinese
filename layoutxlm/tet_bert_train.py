...
path_pickle_train = "train.bert.pickle"
path_pickle_dev = "dev.bert.pickle"
...
test_size = 0.2
epochs = 20
# lr = 1e-5  # 75%, 1epoch, 1error
lr = 2e-5  # 96%, 2epoch, 1error
# lr = 3e-5  # 87%, 2epoch, 1error
# lr = 5e-5  # 87%, 2epoch, 3error
...
def processor(img_or_path, img_size=224, max_len=512, padding="max_length", truncation=True):
    ...
    data_output = tokenizer(text="".join(texts), padding=padding, truncation=truncation, max_length=max_len, stride=0)
...
def preprocess_data(examples):
    ...
    tensor_data = TensorDataset(encoded_inputs_total["image"], encoded_inputs_total["input_ids"], encoded_inputs_total["attention_mask"],
                                encoded_inputs_total["token_type_ids"], encoded_inputs_total["labels"])
    return tensor_data
...
model = BertForSequenceClassification.from_pretrained(model_name_or_path, num_labels=len(id2label))
model.to(device)
